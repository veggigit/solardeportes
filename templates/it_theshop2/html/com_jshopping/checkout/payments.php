...
<table id = "table_payments" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td style = "padding-top:5px; padding-bottom:5px">
       <label style="padding: 8px 0 0;" for = "payment_method_<?php print $payment->payment_id ?>">
       
       	<input style="margin:-3px 2px 0 0;" type = "radio" name = "payment_method" id = "payment_method_<?php print $payment->payment_id ?>" onclick = "showPaymentForm('<?php print $payment->payment_class ?>')" value = "<?php print $payment->payment_class ?>" <?php if ($this->active_payment==$payment->payment_id){?>checked<?php } ?> />
  ...
  <tr id = "tr_payment_<?php print $payment->payment_class ?>" <?php if ($this->active_payment != $payment->payment_id){?>style = "display:none"<?php } ?>>
    <td class = "jshop_payment_method">
	 <div class="control-group">
	   <label class="control-label" for=" <?php print $payment->payment_description?>"> <?php print $payment->payment_description?> </label>
            <div class="controls">
